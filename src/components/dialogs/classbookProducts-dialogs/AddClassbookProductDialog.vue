<template>
   <Modal v-model="showAddModal" modalClass="modal-wrapper">
      <h2 class="add__modal-title">Add new Classbook</h2>
      <div class="modal-content">
        <h3 class="modal-content-name">Classbook name:</h3>
        <input
        class="modal__product__name-input"
        type="text"
        :maxlength="625"
        placeholder="Enter Classbook Name"
        v-model="productAdded.name">
        <h3 class="modal-content-name">Classbook Image</h3>
        ...
        accept="image/*"
        @change="onFilePicked">
        <h3 class="modal-content-name">Classbook description:</h3>
        <textarea placeholder="Enter Classbook Description"
        class="modal__product__price-textarea"
        name="desc"
        id="desc"
        :maxlength="200"
        v-model="productAdded.desc"/>
        <h3 class="modal-content-name">Classbook price:</h3>
        <textarea placeholder="Enter Classbook Price"
        class="modal__product__price-textarea"
        name="price"
        id="price"
        :maxlength="200"
...
export default {
  ...
  methods: {
    ...
    async confirmAdd() {
      ...
      await this.$store.dispatch('createClassbookProduct', classbookProduct);
